...
#+DESCRIPTION: Examples of Org mode markup, export features, Babel, tables, links, drawers, and advanced document elements.
#+KEYWORDS:  Org mode, syntax, markup, documentation
#+LANGUAGE:  en

#+OPTIONS:   H:5 toc:2 num:t p:t

#+SETUPFILE: ~/org/setup/org-macros.setup

# HTML export.
#+SETUPFILE: ~/org/setup/html-theme-readtheorg-home.setup

# LaTeX export.
#+LATEX_CLASS: mcreport
#+SETUPFILE: ~/org/setup/latex-listings.setup
#+SETUPFILE: ~/org/setup/latex-admonitions.setup
#+LATEX_HEADER: \usepackage{amssymb} % Required for examples using \checkmark, \square, etc.
# TODO: Should be in in mcdocument.cls?
...
#+begin_src shell :exports both :results verbatim :eval export
date +"%Y-%m-%d"
#+end_src
...
- ~inline code~ for program code such as ~printf("hi")~, commands such as
  ~git status~, variable names, function names such as ~strlen()~, or language
  keywords.
...
*************** WAIT [#A] Contact the customer                           :phone:
*************** END
...
Additional block types include ~note~, ~seealso~, ~tip~ and ~warning~.
...
{{{hl(Additional macros)}}} are available on [[https://github.com/fniessen/org-macros][GitHub]].
