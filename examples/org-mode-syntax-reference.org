...
- =verbatim text= for exact strings such as =YES=, command-line options such as
  =--force-with-lease=, filenames such as =README.org=, or any text that should be
  reproduced exactly.
...
For example, /italic text containing _underlined text_ within it./
...
By default, emphasis markup cannot span more than *2 consecutive lines*. For
