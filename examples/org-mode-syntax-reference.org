...
- /italic/ for emphasis, book titles, or introducing new terms.
- *bold* for strong emphasis or important information.
- */bold italic/* for very strong emphasis.
- _underlined_ for text that should appear underlined, such as headings or terms
  that require visual emphasis.
- ~inline code~ for program code such as ~printf("hi")~, commands such as ~git
  status~, variable names, function names such as ~strlen()~, or language keywords.
- =verbatim text= for exact strings such as =YES=, command-line options such as
  =--force-with-lease=, filenames such as =README.old=, or any text that should be
  reproduced exactly.
- +deleted text+ for text that has been removed, is obsolete, or should be
  ignored.
...
#+ATTR_HTML: :width 160
[[https://www.youtube.com/shorts/ev1d8wowPuM][file:../images/l-hypnose-c-est-du-fake.png]]
