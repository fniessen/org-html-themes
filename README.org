...
In order to use the ReadTheOrg style in local (not on the net), 
clone org-html-themes from Github, and put the absolute path to the setup dir
at the top of the source, as follows:
#+begin_src org :exports code
,#+SETUPFILE: [absolute_path_to_setup]/theme-readtheorg.setup
#+end_src

Furthermore, you should adjust the directory paths for
'styles', 'jquery', and 'bootstrap' in the file
setup/theme-readtheorg-local.setup
